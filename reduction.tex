...
the corresponding block: the Besov norm $\|u\|_{B^s_{p,q}}$ is the
$\ell^q$ norm of the sequence $2^{js}\|\Delta_j u\|_{L^p}$, and
$B^s_{p,c_0}$ is the subspace on which that sequence tends to zero. The
sharp cutoff costs nothing here, since only $L^2$ quantities enter, and
it is what makes $\PP_{\leq j}u$ a finite-mode test field in the proof of
Lemma~\ref{sec:reduction}.1. The flux $\Pi_j$ is the Fourier-side form
...
below is the Duchon-Robert balance integrated over space, and its limit
$j \to \infty$ is the statement that $D(u)$ has total mass $\A(w_0,w)$
on the window, Proposition~1 of \cite{DucRob00} without its
localization in space; the localized form is used in
...
about the regularity of the flow. The estimate of \cite{CheConFriShv08},
transferred to Navier-Stokes in \cite{CheFriShv10,Shv10}, gives
$\int_{w_0}^{w}\Pi_j\,dt \to 0$ whenever $u \in L^3(w_0,w;
B^{1/3}_{3,c_0})$, so that class admits no averaged floor, and by
Theorem~\ref{sec:reduction}.2(a) a positive defect on the window forces
...
been constructed \cite{BruDeL23,BruColCriDeLSor23}. These are statements
